...
const getRegistrationEditOverride = (
  ...
  try {
    ...
    const keys = [...candidateKeys, urlKey].filter(
    ...
    if (lastEditValue) {
      ...
      const hasSamePatient =
        ...
        keys.includes(lastEdit.rm) ||
        keys.includes(lastEdit.nik) ||
        keys.includes(lastEdit.queue) ||
        keys.includes(lastEdit.patient)
...
function RegistrationDetailPage() {
  ...
  const rawMedicalRecordNo =
    registrationAny?.medicalRecordNo ||
    registrationAny?.medicalRecordNumber ||
    registrationAny?.patient?.medicalRecordNo ||
    registrationAny?.patient?.medicalRecordNumber ||
    registrationAny?.patient ||
    ''

  const rawNationalId =
    registrationAny?.nationalId ||
    registrationAny?.nik ||
    registrationAny?.patientNationalId ||
    registrationAny?.patient?.nationalId ||
    ''

  const rawPatientName =
    registrationAny?.patientName ||
    registrationAny?.fullName ||
    registrationAny?.patientFullName ||
    registrationAny?.patient?.fullName ||
    registrationAny?.patient ||
    ''

  const rawQueue =
    registrationAny?.queue ||
    registrationAny?.queueNumberLabel ||
    registrationAny?.queueLabel ||
    (registrationAny?.queueNumber
      ? `${registrationAny?.clinicCode || registrationAny?.clinic?.code || 'UMUM'}-${String(
          registrationAny.queueNumber,
        ).padStart(3, '0')}`
      : '')

  const editedRegistration = getRegistrationEditOverride([
    String(registrationAny?.id || ''),
    rawMedicalRecordNo,
    rawNationalId,
    rawPatientName,
    rawQueue,
  ...
  const displayMedicalRecordNo = editedRegistration?.rm || rawMedicalRecordNo || '-'
  const displayNationalId = editedRegistration?.nik || rawNationalId || '-'
  const displayPatientName = editedRegistration?.patient || rawPatientName || '-'

  const displayGender =
    editedRegistration?.gender && editedRegistration.gender !== '-'
      ? editedRegistration.gender
      : registrationAny?.gender === 'MALE'
        ? 'Laki-laki'
        : registrationAny?.gender === 'FEMALE'
          ? 'Perempuan'
          : registrationAny?.patient?.gender === 'MALE'
            ? 'Laki-laki'
            : registrationAny?.patient?.gender === 'FEMALE'
              ? 'Perempuan'
              : registrationAny?.gender ||
                registrationAny?.patientGender ||
                registrationAny?.patient?.gender ||
                '-'

  const displayBirthDate =
    editedRegistration?.birthDate && editedRegistration.birthDate !== '-'
      ? formatEditedBirthDate(editedRegistration.birthDate)
      : registrationAny?.birthDate
        ? formatDate(registrationAny.birthDate)
        : registrationAny?.patient?.birthDate
          ? formatDate(registrationAny.patient.birthDate)
          : registrationAny?.patientBirthDate || '-'

  const displayPhone =
    editedRegistration?.phone ||
    registrationAny?.phone ||
    registrationAny?.patientPhone ||
    registrationAny?.patient?.phone ||
    '-'

  const displayAddress =
    editedRegistration?.address ||
    registrationAny?.address ||
    registrationAny?.patientAddress ||
    registrationAny?.patient?.address ||
    '-'

  const displayService =
    editedRegistration?.service ||
    registrationAny?.clinicName ||
    registrationAny?.service ||
    registrationAny?.serviceName ||
    registrationAny?.clinic?.name ||
    '-'

  const displayDoctor =
    editedRegistration?.doctor && editedRegistration.doctor !== '-'
      ? editedRegistration.doctor
      : registrationAny?.doctorName ||
        registrationAny?.doctor?.fullName ||
        registrationAny?.doctor ||
        '-'

  const displayQueue = editedRegistration?.queue || rawQueue || '-'

  const displayStatus =
    editedRegistration?.status ||
    registrationAny?.statusLabel ||
    registrationAny?.status ||
    '-'

  const displayPayerType = mapEditedPayerType(editedRegistration?.payerType)

  const displayInsuranceNo =
    editedRegistration?.insuranceNo && editedRegistration.insuranceNo !== '-'
      ? editedRegistration.insuranceNo
      : registrationAny?.insuranceNo ||
        registrationAny?.guarantorNo ||
        registrationAny?.guarantorNumber ||
        '-'
